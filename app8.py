...
def evaluate_golden_set(kb: KnowledgeBase, golden: pd.DataFrame) -> dict:
    metrics = {
        "n": 0,
    ...
    for _, row in golden.iterrows():
        ...
        if not hits:
            ...
            per_query.append({"query": query, "hit": False, "rrf_top": None})
            continue
        # Hit@K
...
def _render_evaluation(kb: KnowledgeBase) -> None:
    ...
    if st.button("Run evaluation"):
        with st.spinner("Evaluating retrieval + abstention…"):
            metrics = evaluate_golden_set(kb, df)
